...
class DCoM:
    ...
    def calculate_margin(self, model, train_data, data_obj):
        ...
        images_loader = data_obj.getSequentialDataLoader(indexes=all_images_idx, batch_size=self.cfg.TRAIN.BATCH_SIZE, data=train_data)
        clf = model.cuda()
        ranks = []
        n_loader = len(images_loader)

        for i, (x_u, _, _, _) in enumerate(tqdm(images_loader, desc="All images Activations")):
            with torch.no_grad():
                x_u = x_u.cuda(0)
