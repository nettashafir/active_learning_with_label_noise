...
class CoreSetMIPSampling():
    ...
    def get_representation(self, clf_model, idx_set, dataset):
        ...
        for i, (x, _, _, _) in enumerate(tqdm(tempIdxSetLoader, desc="Extracting Representations")):
            with torch.no_grad():
...
def init_centers(X, K):
    ...
    centInds = [0.] * len(X)
    cent = 0
    print('BADGE | #Samps\tTotal Distance')
    logger.info('BADGE | #Samps\tTotal Distance')
    while len(mu) < K:
        if len(mu) == 1:
...
class Sampling:
    ...
    def margin(self, budgetSize, lSet, uSet, model, dataset):
        ...
        """
        num_classes = self.cfg.MODEL.NUM_CLASSES
        assert model.training is False, "Margin | Model expected in eval mode whereas currently it is in {}".format(model.training)
        ...
        n_uLoader = len(uSetLoader)
        print("Margin | len(uSetLoader): {}".format(n_uLoader))
        logger.info("Margin | len(uSetLoader): {}".format(n_uLoader))
        for i, (x_u, _, _, _) in enumerate(tqdm(uSetLoader, desc="uSet Activations")):
            with torch.no_grad():
                x_u = x_u.cuda(0)
        ...
        # index of u_ranks serve as key to refer in u_idx
        print(f"Margin | u_ranks.shape: {u_ranks.shape}")
        logger.info(f"Margin | u_ranks.shape: {u_ranks.shape}")
        # we add -1 for reversing the sorted array
        sorted_idx = np.argsort(u_ranks)[::-1] #argsort helps to return the indices of u_ranks such that their corresponding values are sorted.
        activeSet = sorted_idx[:budgetSize]
    ...
    def badge(self, budgetSize, uSet, model, dataset):
        assert model.training is False, "BADGE | Model expected in eval mode whereas currently it is in {}".format(model.training)

        clf = model.cuda()
        uSetLoader = self.dataObj.getSequentialDataLoader(indexes=uSet, batch_size=int(self.cfg.TRAIN.BATCH_SIZE), data=dataset)
        uSetLoader.dataset.no_aug = True
        n_uLoader = len(uSetLoader)
        print("BADGE | len(uSetLoader): {}".format(n_uLoader))
        logger.info("BADGE | len(uSetLoader): {}".format(n_uLoader))

        embDim = self.cfg.DATASET.REPRESENTATION_DIM
        nLab = 10
        embedding = np.zeros([len(uSet), embDim * nLab])

        bs = uSetLoader.batch_size
        for i, (x_u, _, _, _) in enumerate(tqdm(uSetLoader, desc="uSet Activations")):
            with torch.no_grad():
                x_u = Variable(x_u.cuda(0))
                out, cout = clf(x_u)
                ...
                for j in range(len(x_u)):
                    for c in range(nLab):
                        if c == maxInds[j]:
                            embedding[bs * i + j][embDim * c : embDim * (c+1)] = deepcopy(out[j]) * (1 - batchProbs[j][c])
                        else:
                            embedding[bs * i + j][embDim * c : embDim * (c+1)] = deepcopy(out[j]) * (-1 * batchProbs[j][c])

        chosen = init_centers(embedding, budgetSize)
