...
class TinyImageNet(TinyImageNetClass):
    ...
    """
    def __init__(self, root: str, split: str = 'train', noise_type="clean_label", noise_rate=0.0, exp_dir=None, transform=None, test_transform=None, only_features=False, representation_model="simclr",**kwargs: Any) -> None:
        ...
        assert split in ["train", "val", "test"], f"Split {split} is not supported for TinyImageNet dataset."
        self.train = (self.split == "train")
        self.features = ds_utils.load_features("TINYIMAGENET", train=self.train, representation_model=representation_model)
