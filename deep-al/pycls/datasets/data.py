    ...
    def getDataset(self, save_dir, isTrain=True, isDownload=False, only_features=False, representation_model="simclr", exp_dir=None, project_features=False):
        ...
        #     return cifar100, len(cifar100)
            ...
            if isTrain:
                ...
                tiny = TinyImageNet(save_dir, split='train', noise_type=noise_type, noise_rate=noise_rate, exp_dir=exp_dir, transform=preprocess_steps, test_transform=test_preprocess_steps, only_features=only_features)
            else:
                # tiny = datasets.ImageFolder(save_dir+'/val', transform=preprocess_steps)
                tiny = TinyImageNet(save_dir, split='val', noise_type=noise_type, noise_rate=noise_rate, exp_dir=exp_dir, transform=preprocess_steps, test_transform=test_preprocess_steps, only_features=only_features)
